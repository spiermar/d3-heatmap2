...
export default function () {
    ...
    function heatmap (selection) {
        ...
        if (!data || !Array.isArray(data) || data.length === 0 || !data[0] || !data[0].length) {
            console.log("Error: Invalid or empty data provided to heatmap")
            return heatmap
        }
        ...
        svg.selectAll("g.column")
            ...
            .each(function (d, i) {
                select(this).selectAll("rect")
                    .data(d)
                    .enter().append("rect")
                    .attr("x", function (_d) { return (i * gridSize) + 3 })
                    ...
                    .each(function (d, j) {
                        select(this)
                            .on("mouseover", function (event, d) { return mouseOver(d, i, j) })
                            .on("mouseout", function (event, d) { return mouseOut(d, i, j) })
                            .on("click", function (event, d) { return click(d, i, j) })
                    })
            })
