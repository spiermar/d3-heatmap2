...
function heatmap() {
  ...
  function heatmap2(selection) {
    ...
    if (!colorScale) {
      colorScale = scaleLinear().domain([0, max / 2, max]).range(["#FFFFDD", "#3E9583", "#1F2D86"]);
    }
    ...
    if (yAxisScale || yAxisLabels) {
      if (yAxisScale) {
        var y = scaleLinear().domain(yAxisScale).range([gridSize * rows, 0]);
        svg.append("g").attr("transform", "translate(3, 0)").attr("class", "rowLabel axis").call(axisLeft(y).ticks(yAxisScaleTicks).tickFormat(yAxisTickFormat));
      } else {
    ...
    if (xAxisScale || xAxisLabels) {
      if (xAxisScale) {
        var x = scaleLinear().domain(xAxisScale).range([0, width - calculatedMargin.left - calculatedMargin.right]);
        svg.append("g").attr("transform", "translate(3, 3)").attr("class", "columnLabel axis").call(axisTop(x).ticks(xAxisScaleTicks).tickFormat(xAxisTickFormat));
      } else {
    ...
    if (legendElement) {
      ...
      var legendSvg = select(legendElement).append("svg").attr("width", legendWidth + legendMargin.left + legendMargin.right + 16).attr("height", legendHeight + legendMargin.top + legendMargin.bottom).append("g").attr("transform", `translate(${legendMargin.left + 8}, ${legendMargin.top})`);
      var countScale = scaleLinear().domain([0, max]).range([0, width]);
      var numStops = 10;
      ...
      legendSvg.append("defs").append("linearGradient").attr("id", "legend-traffic").attr("x1", "0%").attr("y1", "0%").attr("x2", "100%").attr("y2", "0%").selectAll("stop").data(range(numStops)).enter().append("stop").attr("offset", function(d, i2) {
      ...
      var xScale = scaleLinear().range([-legendWidth / 2, legendWidth / 2]).domain([0, max]);
      var xAxis = axisBottom().ticks(legendScaleTicks).scale(xScale).tickFormat(legendTickFormat);
